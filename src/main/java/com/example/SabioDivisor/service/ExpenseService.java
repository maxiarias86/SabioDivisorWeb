...
public class ExpenseService {
    ...
    /*
    Si cualquier excepción no chequeada (como RuntimeException o IllegalArgumentException) ocurre durante la ejecución,
    lo que se haya guardado hasta ese momento se revierte con un rollback en la BBDD.
    ...
    public Expense save(Expense expense, Map<Long, Double> payers, Map<Long, Double> debtors) {
        ...
        for (Double value : payers.values()) {
            totalPayed += value;//Suma el total de lo que pagaron los pagadores.
        }
        for (Double value : debtors.values()) {
            totalDebt += value;//Suma el total de lo que deben los deudores.
        }
        if (Math.abs(totalPayed - expense.getAmount()) > 0.01 || Math.abs(totalDebt - expense.getAmount()) > 0.01){//Compara que coincidan. Se deja un mínimo margen de error por redondeo.
        ...
        Map<Long, AppUser> users = new HashMap<>();//Armado de un HashMap con todos los usuarios que participan. Es como un cache para no tener que hacer múltiples consultas al AppUserRepository.
        ...
        for (Long id : payers.keySet()) {//Agregación de todos los pagadores
        ...
        for (Long id: debtors.keySet()) {//Agregacción de los deudores que no están en los pagadores.
        ...
        for (Long id : payers.keySet()) {//Suma lo que pago cada uno. Recorre cada id.
            balances.put(id, payers.get(id));//Agrega al Map balances el ID del pagador y el monto que pagó.
        }
        for (Long id : debtors.keySet()) {//Recorre cada id de los deudores.
            Double amountDebt = debtors.get(id);//Obtiene el monto que debe cada deudor.
            if(balances.containsKey(id)) {//
                ...
                balances.put(id, -amountDebt);//Si no pagó nada directamente lo agrega en negativo porque es una deuda
            }
        }
        ...
        for (Long id : balances.keySet()) {
            ...
            AppUser user = users.get(id);//No se llama del AppUserRepository porque el Map users es como un cache dentro del metodo.

            if (amount < 0) {
                debtorList.add(user);
            } else if (amount > 0) {//Saltea a aquellos que pagaron justo lo que debían, es decir saldo 0.
        ...
        for (int i = 0 ; i < debtorList.size(); i++) {//Itera sobre el ArrayList de deudores
            ...
            Double toDebit = - balances.get(debtorId);//Obtiene el monto de que debe (el "valor" de la "clave" debtorId en el Map balances)
            ...
            for (int j=0 ; j<creditorList.size(); j++ ) {
                ...
                if (toDebit <=0) {//El deudor ya pagó lo que debía, no hace falta seguir recorriendo acreedores para este deudor.
                ...
                //Si todavía hay deuda por pagar y el acreedor tiene saldo positivo, se calcula la deuda a repartir.

                Double amount = Math.min(toDebit,credit);//elige el menor de los dos valores positivos.
                Double installmentAmount = amount / installments;//Calcula el monto de cada cuota de la deuda entre deudor y acreedor.
                ...
                balances.put(creditorId, credit - amount);//Reemplaza la entrada de ese aceedor en el HashMap balances con el nuevo saldo.
                toDebit -= amount;
            }
        }
